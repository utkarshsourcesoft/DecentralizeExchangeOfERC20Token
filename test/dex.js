...
contract('Dex', (accounts) => {
    ...
    beforeEach(async() => {
        ...
        const seedTokenBalance = async (token , trader) => {
            await token.faucet(traderm, amount);
            await token.approve(
                dex.address,
                amount, 
                {from: trader}
    ...
    it('should not withdraws token if balance is low', async () => {
            await dex.deposit(
                web3.utils.toWei('100'),
                web3.utils.fromAscii('TOKEN-DOES-NOT-Exist'),
                 {from: trader1}
        ...
        it('should not  create limit order is dai balance is too low', async() => {0
            await dex.deposit(
                web3.utils.toWei('99'),
                DAI,
                {from: trader1}
        ...
        it('should create market order & match against existing limit order', async () => {
           ...
           await dex.CreateLimitOrder(
            ...
            const balances = await Promise.all([
                ...
                dex.traderBalances(tarder2,DAI),
                dex.traderBalances(trader2,REP),
            ]);
            const orders = await dex.getOrders(REP, SIDE.BUY);
            assert(orders[0].filled === web3.utils.toWei('5'));
            assert(orders[0].toString() === web3.utils.toWei('50'));
            assert(orders[1].toString() === web3.utils.toWei('5'));
            assert(orders[2].toString() === web3.utils.toWei('50'));
            assert(orders[3].toString() === web3.utils.toWei('95'));
